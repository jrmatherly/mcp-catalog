...
Package: {comparison_data["package"]}
Old Version: {comparison_data["old_version"]}
New Version: {comparison_data["new_version"]}

Old version tools ({len(comparison_data["old_tools"])} total):
{json.dumps(comparison_data["old_tools"], indent=2)}

New version tools ({len(comparison_data["new_tools"])} total):
{json.dumps(comparison_data["new_tools"], indent=2)}

Please provide a comprehensive analysis in Markdown format with the following sections:

## 🔧 MCP Tools Analysis: {comparison_data["old_version"]} → {comparison_data["new_version"]}
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert at analyzing API and tool changes, identifying breaking changes, and assessing impact.",
                },
...
def main():
    if len(sys.argv) != 6:
        print(
            "Usage: ai-analyze-tools.py <old_json> <new_json> <package_name> <old_version> <new_version>",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        analysis = analyze_with_openai(
            old_tools, new_tools, package_name, old_version, new_version
        )
        print(analysis)
        sys.exit(0)
    except Exception as e:
