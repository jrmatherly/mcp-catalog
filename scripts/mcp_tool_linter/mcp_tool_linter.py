...
def main():
    ap = argparse.ArgumentParser(
        description="MCP Tool Security Analyzer - Uses LLM to analyze tool definitions for malicious intent"
    )
    ...
    ap.add_argument(
        ...
        dest="fail_on_malicious",
        action="store_false",
        help="Disable exit code 3 for malicious tools",
    )
    ap.add_argument(
        "--openai-api-key",
        help="OpenAI API key (required, or set OPENAI_API_KEY env var)",
    )
    ap.add_argument(
        "--openai-format",
        action="store_true",
    ...
    if not args.openai_format and not api_key:
        print(
            "ERROR: OpenAI API key is required for security analysis. Use --openai-api-key or set OPENAI_API_KEY environment variable"
        )
        print(
            "       Or use --openai-format to only convert tools to OpenAI format without analysis"
        )
        sys.exit(1)
    ...
    for fp in file_list:
        try:
            ...
            worst = max(
                worst, rep.get("overall_severity", "info"), key=lambda s: rank[s]
            )
            reports.append(rep)
        except Exception as e:
    ...
    for rep in reports:
        ...
        for t in rep["tools"]:
            ...
            if "gpt_analysis" in t and "error" not in t["gpt_analysis"]:
                ...
                print("     ⚠️  No GPT analysis available")
    ...
    for rep in reports:
        if "tools" in rep:
            for tool in rep["tools"]:
                if "gpt_analysis" in tool and tool["gpt_analysis"].get(
                    "is_malicious", False
                ):
